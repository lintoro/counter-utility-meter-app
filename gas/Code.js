...
function doGet(e) {
  ...
  const htmlOutput = '<!DOCTYPE html><html><head><meta charset="utf-8">'
    ...
    + '<style>'
    + 'body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #f1f5f9; color: #1e293b; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; padding: 16px; box-sizing: border-box; }'
    + '.card { background: white; max-width: 420px; width: 100%; padding: 32px 24px; border-radius: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.08); text-align: center; }'
    + '.icon { font-size: 52px; margin-bottom: 12px; }'
    + 'h2 { font-size: 20px; margin: 0 0 10px; color: #0f172a; }'
    + 'p { font-size: 15px; color: #64748b; line-height: 1.6; margin: 0 0 24px; }'
    + '.badge { display: inline-block; background: ' + (isOk ? '#e0f2fe' : '#fee2e2') + '; color: ' + (isOk ? '#0369a1' : '#b91c1c') + '; padding: 6px 14px; border-radius: 9999px; font-size: 13px; font-weight: bold; margin-bottom: 16px; }'
    + '.btn { display: block; background: #2563eb; color: white; padding: 14px; border-radius: 12px; text-decoration: none; font-weight: bold; font-size: 16px; }'
    + '</style></head><body>'
    + '<div class="card">'
    + '<div class="icon">' + (isOk ? '🎉🤖' : '⚠️') + '</div>'
    + '<div class="badge">' + (isOk ? '操作執行成功' : '處理提示') + '</div>'
    + '<h2>' + mainTitle + '</h2>'
    + '<p>' + detailText + '</p>'
    + '<a href="javascript:history.back();" class="btn" style="background:#10b981; margin-bottom:12px;">🔙 立即返回抄表 App</a>'
    + '<a href="javascript:window.close();" style="display:block; color:#64748b; font-size:14px; text-decoration:none;">關閉此視窗</a>'
    + '</div>'
    + '<script>'
    + 'setTimeout(function() {'
